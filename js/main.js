...
const addRuntime = () => {
    ...
    if ($runtimeCount.length) {
        ...
        const showDateTime = function () {
            const BirthDay = new Date(publishDate)
            ...
            const diffYears = Math.floor(timeold / years);
            const diffDays = Math.floor((timeold / days) - diffYears * 365);
            const diffHours = Math.floor((timeold - (diffYears * 365 + diffDays) * days) / hours);
            const diffMinutes = Math.floor((timeold - (diffYears * 365 + diffDays) * days - diffHours * hours) /
                minutes);
            const diffSeconds = Math.floor((timeold - (diffYears * 365 + diffDays) * days - diffHours * hours -
                diffMinutes * minutes) / seconds);
            $runtimeCount.text( diffYears+"年"+ diffDays+GLOBAL_CONFIG.runtime_unit+ diffHours+"小时"+diffMinutes +"分钟"+diffSeconds+"秒")
        }
        showDateTime()
