...
function bot_ui_ini() {
    ...
    botui.message.add({delay: 800, content: "嗨，你好👋"}).then(function () {
        botui.message.add({delay: 1100, content: "这里是itcnw3c.com"}).then(function () {
            botui.message.add({delay: 1100, content: "一个帅气的男孩子~"}).then(function () {
                botui.action.button({
                    delay: 1600,
    ...
    var sure = function () {
        ...
        botui.message.add({
            delay: 600,
            content: "![...](/img/aoloi.png)"
        })
        /// https://view.moezx.cc/images/2018/05/06/a1c4cd0452528b572af37952489372b6.md.jpg
    }, secondpart = function () {
        botui.message.add({delay: 1500, content: "毕业于重庆交通大学"}).then(function () {
            botui.message.add({delay: 1500, content: "是在计算机专业学习的程序猿"}).then(function () {
                botui.message.add({delay: 1200, content: "将敲代码看成一种快乐"}).then(function () {
                    botui.message.add({
                        delay: 1500,
                        content: "主修 Java 和 Python,略知 linux,偶尔也搞 #C/HTML/CSS/JavaScript/Spring clound alibaba"
                    }).then(function () {
                        botui.message.add({
                            delay: 1500,
                            content: "1994年中国制造，长170cm，净重56kg。"
                        }).then(function () {
                            botui.message.add({delay: 1800, content: "本人没什么优点的优点就是没有缺点本人基本全是缺点的缺点就是只有优点如果靓仔是一种罪，我已经罪犯滔天;如果有型是一种错，我已经一错再错;如果聪明要受惩罚，我岂不是要千刀万剐;如果谦虚都要受折磨，我又点能逃得过。😎 "}).then(function () {
                                botui.action.button({
                                    delay: 1100,
                                    action: [{text: "为什么搭建了这个博客呢？", value: "why-mashiro"}]
                                }).then(function (a) {
                                    thirdpart()
                                })
                            })
        ...
        botui.message.add({delay: 1e3, content: "搭建这个博客是为了记录一些笔记,以及学习过程中踩过的坑。。。"}).then(function () {
            botui.action.button({delay: 1500, action: [{text: "域名为什么叫itcnw3c呢？🤔", value: "why-cat"}]}).then(function (a) {
                fourthpart()
            })
        })
    }, fourthpart = function () {
        botui.message.add({delay: 1e3, content: "当时，在万网上看到了这个域名一眼就相中了。可能是缘分吧。。。"}).then(function () {
            botui.message.add({delay: 1100, content: "其实是囊中羞涩，主要是这个域名便宜呀。"}).then(function () {
                botui.action.button({
                    delay: 1500,
                    action: [{text: "博主做程序员会脱发吗？", value: "why-domain"}]
                }).then(function (a) {
                    fifthpart()
                })
            })
        ...
        botui.message.add({delay: 1e3, content: "额~其他人我不知道，反正我没有"}).then(function () {
            botui.message.add({delay: 1600, content: "那么，相遇就是缘分，赏个赞吧😜"})
        })
    }
}
